...
function SWEP:SecondaryAttack()
    self:SetNextSecondaryFire(CurTime() + 8) -- 3 second cooldown
    
    if SERVER then
        -- Play jump sound
        self.Owner:EmitSound("player/fatkid/hale/halelunge.mp3", 100, 100)
        
        -- Get forward direction but keep some upward momentum
        local forward = self.Owner:GetAimVector()
        forward.z = math.max(forward.z, 0.3) -- Ensure some upward boost
        forward:Normalize()
        
        -- Apply the impulse
        local boostStrength = 600
        self.Owner:SetVelocity(forward * boostStrength)
        ...
        -- Reset hold type after animation
